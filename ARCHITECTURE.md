This document aims to outline at a high level the various parts that make up TrackAsia Native and how they work together.

# Repository structure

TrackAsia Native uses a monolithic repository that houses both core C++ code and code that wraps the C++ core with SDKs for Android, iOS, macOS, Node.js, and Qt. A "monorepo" allows us to:
...
- `expression-test` contains tests for the expression feature in the map style (see more details about expression [here](https://trackasia.com/trackasia-style-spec/expressions/).
...
The TrackAsia Native build system uses a variety of tools.
...
npm is a package manager for Node.js. TrackAsia Native uses it to pull in several development dependencies that happen to be packaged as node modules -- mainly for testing needs.

## Mason

[Mason](https://github.com/mapbox/mason) is Mapbox's own cross platform C/C++ package manager. TrackAsia Native uses mason packages as a source of precompiled binaries for third-party dependencies such as Boost, RapidJSON, and SQLite, and Mapbox's own C++ modules such as [earcut.hpp](https://github.com/mapbox/earcut.hpp) and [geojson-vt-cpp](https://github.com/mapbox/geojson-vt-cpp). It is also used to obtain a toolchain for Android platform cross-compiliation.
...
[gyp](https://gyp.gsrc.io/) is a build system originally created for Chromium and since adopted by Node.js. In TrackAsia Native it's used to build the core C++ static library, the Node.js platform module, and the shared JNI module for Android.
...
The "Style" component of TrackAsia Native contains an implementation of the [TrackAsia Style Spec](https://trackasia.com/trackasia-style-spec/), defining what data to draw, the order to draw it in, and how to style the data when drawing it.
...
At runtime, TrackAsia Native uses the following threads:
