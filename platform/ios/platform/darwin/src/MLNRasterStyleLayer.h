...
/**
 ...
 loaded by an `MLNRasterTileSource` object or raster images loaded by an
 `MLNImageSource` object. For example, you could use a raster style layer to
 ...
 register an image for use as an icon or pattern, use the `-[MLNStyle
 setImage:forName:]` method. To configure a point annotation’s image, use the
 `MLNAnnotationImage` class.
