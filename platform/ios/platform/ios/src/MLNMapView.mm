...
const NSTimeInterval MLNBackgroundSnapshotImageInterval = 60.0;

/// The delay after the map has idled before a background snapshot is attempted.
const NSTimeInterval MLNBackgroundSnapshotImageIdleDelay = 3.0;
...
- (void)attemptBackgroundSnapshot {
    ...
    if (lastSnapshotTime == 0.0 || (now - lastSnapshotTime > MLNBackgroundSnapshotImageInterval)) {
        MLNLogDebug(@"Taking snapshot");
...
- (void)queueBackgroundSnapshot {
    ...
    [self performSelector:@selector(attemptBackgroundSnapshot)
               withObject:nil
               afterDelay:MLNBackgroundSnapshotImageIdleDelay];
}
