...
final class LocationCameraController {
  ...
  private void setPadding(double[] padding) {
    ...
    transform.moveCamera(trackasiaMap, CameraUpdateFactory.paddingTo(padding), null);
    onCameraMoveInvalidateListener.onInvalidateCameraMove();
  }
  ...
  private final TrackAsiaAnimator.AnimationsValueChangeListener<double[]> paddingValueListener =
  ...
  Set<AnimatorListenerHolder> getAnimationListeners() {
    ...
    holders.add(new AnimatorListenerHolder(TrackAsiaAnimator.ANIMATOR_PADDING, paddingValueListener));
    return holders;
  }
