...
open class MapSnapshotter(context: Context, options: Options) {
    ...
    private fun getAttributionMeasure(
        ...
        return AttributionMeasure.Builder().setSnapshot(
            snapshot,
        ).setLogo(logo.large).setLogoSmall(logo.small).setTextView(longText).setTextViewShort(shortText)
    ...
    private fun createAttributionString(
        ...
        val attributionParser =
            AttributionParser.Options(
                context,
            ).withAttributionData(*mapSnapshot.attributions).withCopyrightSign(false).withImproveMap(false).build()
