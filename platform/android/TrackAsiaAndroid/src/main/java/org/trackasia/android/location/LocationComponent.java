...
import static org.trackasia.android.location.LocationComponentConstants.DEFAULT_TRACKING_PADDING_ANIM_DURATION;
...
public final class LocationComponent {
  ...
  public void paddingWhileTracking(double[] padding, long animationDuration,
    ...
    locationAnimatorCoordinator.feedNewPadding(padding, trackasiaMap.getCameraPosition(), animationDuration, callback);
  }

  /**
   * Cancels animation started by {@link #paddingWhileTracking(double[], long, TrackAsiaMap.CancelableCallback)}.
   */
   ...
   * <pre>
   * {@code
   * TrackAsiaMap.addOnCameraIdleListener(new TrackAsiaMap.OnCameraIdleListener() {
   *   {@literal @}Override
   *   public void onCameraIdle() {
   *     double zoom = TrackAsiaMap.getCameraPosition().zoom;
   *     int maxAnimationFps;
   *     if (zoom < 5) {
   *       maxAnimationFps = 3;
   *     } else if (zoom < 10) {
