...
class CameraPositionTest {
    ...
    fun testTypedArrayBuilder() {
        ...
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.trackasia_MapView_trackasia_cameraBearing,
                0.0f
            )
        ).thenReturn(bearing)
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.trackasia_MapView_trackasia_cameraTargetLat,
                0.0f
            )
        ).thenReturn(latitude)
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.trackasia_MapView_trackasia_cameraTargetLng,
                0.0f
            )
        ).thenReturn(longitude)
        Mockito.`when`(typedArray.getFloat(R.styleable.trackasia_MapView_trackasia_cameraZoom, 0.0f))
            .thenReturn(zoom)
        Mockito.`when`(typedArray.getFloat(R.styleable.trackasia_MapView_trackasia_cameraTilt, 0.0f))
            .thenReturn(tilt)
