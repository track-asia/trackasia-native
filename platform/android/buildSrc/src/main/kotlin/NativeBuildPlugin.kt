...
fun Project.nativeBuild(nativeTargets: List<String>) =
    this.extensions.getByType<BaseExtension>().run {
        ...
        //
        // When invoking from the command line or to override the device default, set `-Ptrackasia.abis=...` to
        ...
        var abi = "all"
        if (!project.hasProperty("android.injected.invoked.from.ide") && project.hasProperty("trackasia.abis")) {
            abi = project.property("trackasia.abis") as String
        }

        if (abi != "none") {
            externalNativeBuild {
                cmake {
                    path = file("../TrackAsiaAndroid/src/cpp/CMakeLists.txt")
