...
Before we jump into the coordinate system of TrackAsia Native GL, let's
...
will project a WGS 84 point to a map tile rendered in TrackAsia GL
Native.
...
*World* to denote the world TrackAsia Native GL renders. The word *world*
...
TrackAsia GL Native renders map tiles in a range of zoom levels on a 3D
...
TrackAsia GL requires a series of coordinate transformations to render a
...
1.  Design and define model in the *Local Space*. We call anything that
    ...
    its own coordinate system. For TrackAsia Native GL, in local space
...
Going forward, scoping our discussion only to TrackAsia GL Native, we
...
EPSG:3857 is used by TrackAsia GL Native as a default coordinate system
...
tile, we need to know what is the *extent* of the tile. TrackAsia GL
Native follows Mapbox Vector Tile (MVT) spec. Following said spec,
TrackAsia GL Native internally normalizes each tile to an *extent* of
...
only one object. And in TrackAsia GL, this plane has an origin of (0,0),
positioned on the top left.

## Device Coordinates

World space for TrackAsia GL Native contains a plane in 3D with all the
tiles for any specific zoom level. Map tiles are hierarchical in nature.
As in they have different zoom levels. TrackAsia GL Native internally
...
*Projection Matrix*. In TrackAsia GL Native, a camera initialization
...
matrix. And TrackAsia GL Native uses a *perspective projection*.
...
shaders that TrackAsia Native GL at the time of writing uses 4
...
On top of this, TrackAsia GL Native uses a field of view of 36.87 degrees
...
the tile coordinates we wish to render in TrackAsia GL Native but only in
...
[^11]: There are other coordinate systems such as EPSG:54001 that uses
    ...
    document focuses on EPSG:3857 because TrackAsia GL Native uses it by
    default.
...
[^15]: The piece of code we run on GPU is called a shader. We will see
    more how shaders influence TrackAsia GL Native rendering later in the
    document.
