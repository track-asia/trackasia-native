...
test('Popup#getElement returns a .TrackAsiaGL-popup element', (t) => {
    ...
    t.ok(popup.isOpen());
    t.ok(popup.getElement().classList.contains('TrackAsiaGL-popup'));
    t.end();
});

test('Popup#addTo adds a .TrackAsiaGL-popup element', (t) => {
    const map = createMap(t);
    const popup = new Popup()
        .setText('Test')
        .setLngLat([0, 0])
    ...
    t.ok(popup.isOpen());
    t.equal(map.getContainer().querySelectorAll('.TrackAsiaGL-popup').length, 1);
    t.end();
});
...
test('Popup has no close button if closeButton option is false', (t) => {
    ...
    t.equal(popup.getElement().querySelectorAll('.TrackAsiaGL-popup-close-button').length, 0);
    t.end();
});
...
test('Popup content can be set via setHTML', (t) => {
    ...
    t.equal(popup.getElement().querySelector('.TrackAsiaGL-popup-content').innerHTML, "<span>Test</span>");
    t.end();
});
...
test('Popup content can be set via setDOMContent', (t) => {
    ...
    t.equal(popup.getElement().querySelector('.TrackAsiaGL-popup-content').firstChild, content);
    t.end();
});
...
test('Popup anchors as specified by the anchor option', (t) => {
    ...
    t.ok(popup.getElement().classList.contains('TrackAsiaGL-popup-anchor-top-left'));
    t.end();
});
...
[
    ...
    test(`Popup automatically anchors to ${anchor}`, (t) => {
        ...
        t.ok(popup.getElement().classList.contains(`TrackAsiaGL-popup-anchor-${anchor}`));
        t.end();
    });
...
test('Popup automatically anchors to top if its bottom offset would push it off-screen', (t) => {
    ...
    t.ok(popup.getElement().classList.contains('TrackAsiaGL-popup-anchor-top'));
    t.end();
});
...
test('Popup#addTo is idempotent (#1811)', (t) => {
    ...
    t.equal(popup.getElement().querySelector('.TrackAsiaGL-popup-content').textContent, 'Test');
    t.end();
});
...
test('Popup#remove is idempotent (#2395)', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.TrackAsiaGL-popup').length, 0);
    t.end();
});
...
test('Pointer-tracked popup is tagged with right class', (t) => {
    ...
    t.equal(popup._container.classList.value.includes('TrackAsiaGL-popup-track-pointer'), true);
    t.end();
});
...
test('Pointer-tracked popup can be repositioned with setLngLat', (t) => {
    ...
    t.deepEqual(popup._pos, map.project([0, 0]));
    t.equal(popup._container.classList.value.includes('TrackAsiaGL-popup-track-pointer'), false);
    t.end();
});
