...
test('AttributionControl appears in bottom-right by default', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.TrackAsiaGL-ctrl-bottom-right .TrackAsiaGL-ctrl-attrib').length, 1);
    t.end();
});
...
test('AttributionControl appears in the position specified by the position option', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.TrackAsiaGL-ctrl-top-left .TrackAsiaGL-ctrl-attrib').length, 1);
    t.end();
});
...
test('AttributionControl appears in compact mode if compact option is used', (t) => {
    ...
    t.equal(container.querySelectorAll('.TrackAsiaGL-ctrl-attrib.TrackAsiaGL-compact').length, 1);
    ...
    map.addControl(attributionControl);
    t.equal(container.querySelectorAll('.TrackAsiaGL-ctrl-attrib:not(.TrackAsiaGL-compact)').length, 1);
    t.end();
});
...
test('AttributionControl is hidden if empty', (t) => {
    ...
    const checkEmptyFirst = () => {
        ...
        t.equal(container.querySelectorAll('.TrackAsiaGL-attrib-empty').length, 1, 'includes empty class when no attribution strings are provided');
    ...
    const checkNotEmptyLater = () => {
        ...
        t.equal(container.querySelectorAll('.TrackAsiaGL-attrib-empty').length, 0, 'removes empty class when source with attribution is added');
        t.end();
    };
