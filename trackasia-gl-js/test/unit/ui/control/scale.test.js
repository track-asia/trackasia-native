...
test('ScaleControl appears in bottom-left by default', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.TrackAsiaGL-ctrl-bottom-left .TrackAsiaGL-ctrl-scale').length, 1);
    t.end();
});
...
test('ScaleControl appears in the position specified by the position option', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.TrackAsiaGL-ctrl-top-left .TrackAsiaGL-ctrl-scale').length, 1);
    t.end();
});
