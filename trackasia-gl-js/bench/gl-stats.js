...
const TrackAsiaGLJSSrc = fs.readFileSync('dist/mapbox-gl.js', 'utf8');
const TrackAsiaGLCSSSrc = fs.readFileSync('dist/mapbox-gl.css', 'utf8');
const benchSrc = fs.readFileSync('bench/gl-stats.html', 'utf8');
const {execSync} = require('child_process');

const benchHTML = benchSrc
    .replace(/<script src="..\/dist\/mapbox-gl.js"><\/script>/, `<script>${TrackAsiaGLJSSrc}</script>`)
...
(async () => {
    ...
    stats["bundle_size"] = TrackAsiaGLJSSrc.length + TrackAsiaGLCSSSrc.length;
    stats["bundle_size_gz"] = zlib.gzipSync(TrackAsiaGLJSSrc).length + zlib.gzipSync(TrackAsiaGLCSSSrc).length;
